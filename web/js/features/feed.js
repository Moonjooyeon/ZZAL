...
export function card(z) {
  ...
  return `<article class="pin"><button class="shot" onclick="openDetail(${z.id})" aria-label="${t} 자세히 보기"><div class="art">${z.art}</div></button><div class="meta"><h3>${t}</h3><button class="save" aria-label="${t} ${on ? '저장 해제' : '저장'}" aria-pressed="${on}" onclick="toggle(${z.id})">${icons.save}</button></div><p class="tags">${z.tags.map(x => '#' + esc(x)).join(' &nbsp; ')}</p></article>`;
}
